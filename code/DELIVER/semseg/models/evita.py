...
class EvitaArchitecture(nn.Module):
    ...
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768],
        ...
        stem_evt = nn.Sequential(
            nn.Conv2d(3, dims[0] // 4, kernel_size=3, stride=2, padding=1),
...
def Evita_Pico(pretrained=False, **kwargs):
    model = EvitaArchitecture(dims=[16, 32, 64, 128], mlp_ratios=[4, 4, 4, 4], depths=[2, 2, 2, 2], num_heads=[1, 2, 4, 8], windows=[0, 7, 7, 7], drop_path_rate=0.00, **kwargs)
...
class Evita(nn.Module):
    def __init__(self, backbone: str = 'Evita_Large', num_classes: int = 25, pretrained_path: str = None) -> None:
        ...
        BACKBONE_REGISTRY = {
            ...
            'Evita_Large': {'builder': Evita_Large, 'channels': [96, 192, 288, 576], 'embed_dim': 512},
        }
    ...
    def forward(self, x: list) -> Tensor:
        ...
        logits = F.interpolate(logits, size=x[0].shape[2:], mode='bilinear', align_corners=False)
        return logits

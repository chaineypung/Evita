...
def build_norm(cfg, num_features):
    ...
    if norm_type == 'BN':
        return nn.BatchNorm2d(num_features, **cfg_)
    elif norm_type == 'GN':
        num_groups = cfg_.get('num_groups', 32)
        return nn.GroupNorm(num_groups, num_features)
    elif norm_type == 'LN':
        return nn.LayerNorm(num_features, **cfg_)
    elif norm_type == 'SyncBN':
        return nn.SyncBatchNorm(num_features, **cfg_)
    else:
...
class BasicConvModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, padding=0, 
        ...
        self.norm = build_norm(norm_cfg, out_channels) if norm_cfg is not None else nn.Identity()
        ...
        if act_cfg is None:
            ...
            raise NotImplementedError(f"not support {act_cfg.get('type')}")
...
class Hamburger(nn.Module):
    def __init__(self, ham_channels=512, ham_kwargs=dict(), norm_cfg=None):
        ...
        self.ham_in = BasicConvModule(ham_channels, ham_channels, 1, norm_cfg=None, act_cfg=None)
        self.ham = NMF2D(ham_kwargs)
        self.ham_out = BasicConvModule(ham_channels, ham_channels, 1, norm_cfg=norm_cfg, act_cfg=None)
...
class LightHamHead(nn.Module):
    def __init__(self, in_channels=[64, 128, 320, 512], ham_channels=512, ham_kwargs=dict(), num_classes=25):
        ...
        self.squeeze = BasicConvModule(sum(self.in_channels), self.ham_channels, 1, 
                                       norm_cfg=self.norm_cfg, act_cfg=self.act_cfg)
        self.hamburger = Hamburger(ham_channels, ham_kwargs, self.norm_cfg)
        self.align = BasicConvModule(self.ham_channels, self.channels, 1, 
                                     norm_cfg=self.norm_cfg, act_cfg=self.act_cfg)
